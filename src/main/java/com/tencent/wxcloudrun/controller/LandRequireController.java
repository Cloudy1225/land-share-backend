...
class LandRequireController {
    ...
    @Autowired
    public LandRequireController(LandRequireService landRequireService) {
    ...
    {
        logger.info("/landRequire/createLandRequire post request, 目的：新建土地发布，用户openid: {}", openid);
    ...
    public Response getMyLandRequires(@RequestHeader("X-WX-OPENID") String openid){
        logger.info("/landRequire/getMyLandRequires get request, 目的：获取我的土地需求发布，用户openid: {}", openid);

        ArrayList<LandRequireVO> myLandRequires = landRequireService.getMyLandRequires(openid);
        return Response.buildSuccess("我的土地发布获取成功", myLandRequires);
    }
    ...
    public Response updateLandRequire(@RequestBody LandRequireVO landRequireVO, @RequestHeader("X-WX-OPENID") String openid){
        logger.info("/landRequire/updateLandRequire post request, 目的：更新土地需求发布信息，用户openid: {}", openid);
    ...
    public Response delLandRequires(@RequestParam Integer lrid, @RequestHeader("X-WX-OPENID") String openid){
        logger.info("/landRequire/deleteLandRequire get request, 目的：删除指定土地需求发布，用户openid: {}，土地lid: {}", openid, lrid);
    ...
    public Response getLandRequires(@RequestBody LandFilterDto landFilterDto){
        logger.info("/landRequire/getLandRequires post request, 目的：按筛选条件获取10条土地发布");

        ArrayList<LandRequireVO> landRequires = landRequireService.getLandRequiresByFilters(landFilterDto);

        return Response.buildSuccess("土地发布获取成功", landRequires);
    }
